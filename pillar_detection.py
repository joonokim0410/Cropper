...
parser.add_argument("-s", "--source_dir", default="./", help="Input source vid directory")
parser.add_argument("-i", "--ivtc_dir", default="./ivtc", help="Input ivtc vid directory")
...
def main():
    ...
    for types in vid_types :
        source_list += sorted(glob.glob(os.path.join(source_dir, types)))
    ...
    for i, fpath in enumerate(ivtc_list):
        vid_name = os.path.basename(fpath)[:-4]
        vid_num = len(ivtc_list)
